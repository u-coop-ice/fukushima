<?php
function smarty_block_htkt_categories($params, $content, &$smarty, &$repeat) {
	$categories = array();
	// データベース関連の情報を得る
...
SELECT c.*,COUNT(e.id) AS entry_count
 FROM htkt_category AS c
 LEFT JOIN htkt_entry AS e ON c.id = e.category_id
...
SELECT c.*,COUNT(e.id) AS entry_count
 FROM htkt_category AS c
 LEFT JOIN (SELECT e1.id,e1.category_id FROM htkt_entry AS e1 WHERE e1.publish = 1 ) AS e ON c.id = e.category_id
