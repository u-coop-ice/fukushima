...
{/capture}
{* ページタイトル 開始 *}
{assign var="page_title" value="入金済＋入金確認メール未送信 一覧"}
{* ページタイトル 終了 *}
...
<div id="search">
<form method="PUT" id="searchForm" action="{$self}?mode=list_paid_completed" class="filter_entry form-inline">
...
<input type="hidden" name="mode" value="list_paid_completed" />
<button type="submit" class="btn btn-primary" value="検索">検索</button>
</form>
</div>

{if $sent}
<p class="alert alert-info">入金確認メールを送信しました。</p>
{/if}


{*order_page_navi_setuppaid=1 not_sendmail_paid_completed=1*}
{orders paid=1 not_sendmail_paid_completed=1 per_page=50 status=1 nocard=1}
{if $order_header}
<p>{$order_count}件中の{$first_order_no}〜{$last_order_no}件目</p>


<form method="post" id="exportForm" action="{$self}?mode=sendmail_paid_completed">
...
<td>{if $order['category_paid_completed_message']}<input type="checkbox" id="app_id[{$order['id']}]" name="app_id[]" value="{$order['id']}" />{/if}</td>
<td>
...
<p><button class="btn btn-primary" type="submit" id="" name="export" value="「入金を確認しました」メールを送信する" ><i class="fa fa-fw fa-envelope"></i>「入金を確認しました」メールを送信する</button></p>
</form>
...
カテゴリ設定で入金完了メールの設定が無い場合は、この機能は使えません。
<br />この一覧で選択できない注文は、個々の注文ページから操作ください。</p>
...
{if $no_order}
<p class="alert alert-info">注文登録が見つかりませんでした。</p>
{/if}
{if $no_category}
<p class="alert alert-info">権限がありません。</p>
{/if}
{if $db_error}
<p class="alert alert-danger">注文情報の読み込みに失敗しました。</p>
{/if}
