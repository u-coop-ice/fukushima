...
{* 記事本体 開始 *}
<h4 class="page_title">{$page_title}<span class="tag micro">ヤマト版</span></h4>
...
<form method="post" action="{$self}?mode=export_excel_dev" class="filter_entry">
...
{html_radios name="status" options=$statusOrderList output=$statusOrderList selected=$condition['status'] assign=radioTags}
...
<div>
<label class="checked"><input type="radio" name="ship" value="1" checked="checked" />配送のみ</label>
</div>
</div>
<div class="clear"></div>
...
<div class="radio-group clearfix">
<div>
<label class="checked"><input type="radio" id="format_all" name="format" value="ALL" checked="checked" /> クロネコヤマト送り状用</label>
</div>
</div>
<div class="clear"></div>
