...
<link type="text/css" href="/js/jquery/jquery-ui-1.11.4/bootstrap/jquery-ui.custom.css" rel="stylesheet" media="screen,print" />
...
カテゴリの{if $copy}複製{else if $view_category_id}編集{else}新規作成{/if}
{/capture}
...
<td>
<input type="text" name="denomination" id="denomination" class="form-control validate[required]" value="{$category['denomination']}" /></td>
</tr>
<tr>
...
<td><input type="text" name="pressmail" id="pressmail" class="form-control" value="{$category['pressmail']}" /><p class="help-block">空欄の場合、問い合わせフォームのURLが記載されます。</p></td>
</tr>

<tr>
<th>概要（webフォーム記載側）</th>
<td><textarea name="description_web" id="description_web" cols="50" rows="5" class="form-control">{$category['description_web']}</textarea></td>
</tr>
<tr>
<th>概要（自動返信メール記載側）</th>
...
<td><label><input type="tel" name="sort_order" id="sort_order" class="form-control" value="{$category['sort_order']}" /></label>
</td>
</tr>

<tr>
<th>フォームURL</th>
<td>{if $copy}<p class="alert alert-info">保存するとURLが発行されます。</p>{else}{if $category['denomination']}
<code class="btn-copy-clipboard" title="クリップボードに保存する"><i class="fa fa-fw fa-clipboard"></i>{$init_url}app/{$smarty.const.COMPONENT}/{$smarty.const.PART}/</code><br />
...
キャンセル期限：
<input type="text" name="date_limit_cancel" class="datetime form-control" id="date_limit_cancel" value="{$category['date_limit_cancel']|default:'2038-01-01 23:59:59'}" /> <span class="reset"><a class="btn btn-primary btn-sm"><i class="fa fa-remove"></i>リセット</a></span>
*}
</td>
</tr>
...
{*<div class="radio-group clearfix">
{html_radios name="onduplicate" id="onduplicate" options=$onoffList selected=$category['onduplicate']|default:0 assign=radioTags}
{section name=radioButtons loop=$radioTags}
<div>{$radioTags[radioButtons]}</div>
{/section}
</div>
...
<div class="radio-group radio">
{if $copy}
{html_radios name="archived" options=$onoffList selected=0 assign=radioTags}
{else}
