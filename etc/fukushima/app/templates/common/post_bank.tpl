...
<input type="text" id="bank_account" name="bank_account" class="form-control input-lg validate[{if $methods['bank']['use']==2}required,{/if}custom[number],minSize[8]]" maxlength="8" placeholder="（半角数字 8桁）"  value="{if $post['bank_account']}{$post['bank_account']|string_format:"%08d"}{/if}" />
</div>
</div>
