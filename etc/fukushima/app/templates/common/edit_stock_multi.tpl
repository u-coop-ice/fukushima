...
{$methods['stock_multi']['title']}{if $methods['stock_multi']['use']==2}<span class="label label-danger">必須</span>{else}<span class="label label-default">任意</span>{/if}
</label>
<div class="col-sm-9">
...
{else}
<label><input type="radio" name="stock_multi" value="{$key}" {if $post["stock_multi"]==$key}checked="checked"{/if}>{$key}</label>
{/if}
</div>
{/foreach}
{/if}
