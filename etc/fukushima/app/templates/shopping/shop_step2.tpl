...
if(setDays[0]){
	delivery_starts[i-1] = setDays[0];
}
if (setDays[setDays.length-1]){
	delivery_ends[i-1] = setDays[setDays.length-1];
}
...
function setNoshi() {
	for (i=1; i<=n;i++){
		...
		$('#noshi_name'+i).prop("disabled",false);
		} else if ($('#noshi'+i).val() == "不要"){
		$('#noshi_other'+i).prop("disabled",true).removeClass("validate[required]");
		$('#noshi_name'+i).prop("disabled",true);
		} else if ($('#noshi'+i).val()) {
		$('#noshi_other'+i).prop("disabled",true);
		$('#noshi_name'+i).prop("disabled",false);
		} else {
		$('#noshi_other'+i).prop("disabled",true).removeClass("validate[required]");
		$('#noshi_name'+i).prop("disabled",true);
...
<form id="theForm" method="post" class="form-horizontal" action="{$self}?mode=buy_confirm">
<table class="inputForm">
<col style="width:20%" />
<col style="width:80%" />
{if !$post['ship_flag']}
...
{if $methods['age']['use']}
<tr>
<th>年齢{if !$address["ship_age"]}<span class="label label-danger">必須</span>{/if}</th>
...
{if $no_adult_ship_age_err}<p class="alert alert-danger">*20歳以上の年齢であることが確認できない場合には、酒類のお申込はお受けできません。</p>{/if}
<p class="help-block">※酒類をお買上の場合は必ず選択下さい。<br />
...
{if $methods['age']['use']}
<tr>
<th>年齢<span class="label label-danger">必須</span></th>
<td>
<div class="radio">
{html_radios name="ship_age" options=$ageCheckList class="validate[required]" checked=$post['ship_age']}
</div>
{if $no_adult_ship_age_err}<p class="label label-danger">*20歳以上の年齢であることが確認できない場合には、酒類のお申込はお受けできません。</p>{/if}
<p class="help-block">※酒類をお買上の場合は必ず選択下さい。<br />
未成年の飲酒は法律で禁止されています。20歳以上の年齢であることが確認できない場合には、酒類のお申込はお受けできません。</p>
{if $ship_age_err}<span class="must_view">*必須項目です</span>{/if}
...
{if $itm["nominate"]}
<input type="text" class="ship_date datepicker form-control input-lg" id="ship_date{$ctr}" name="ship_date{$ctr}" readonly="readonly" value="{$item["ship_date"]}" />
{if $itm["send_date"]}<br />（お届け期間：{$itm["send_date"]}）{/if}
{else}
<input type="hidden" id="ship_date{$ctr}" name="ship_date{$ctr}" value="日付指定不可" />
<p class="form-control-statics">日付指定不可{if $itm["send_date"]}（お届け期間：{$itm["send_date"]}）{/if}</p>
{/if}
</div>
</div>
<div class="form-group">
...
<input class="form-control input-lg" type="text" id="noshi_name{$ctr}" name="noshi_name{$ctr}" value="{$item["noshi_name"]}" />（のしの名入れ等）
{/if}
</div>
</div>
{/if}
...
<button class="btn btn-primary btn-block" name="step3" type="submit" value="お支払い方法の入力へ進む">お支払い方法の入力へ進む<i class="fa fa-fw fa-chevron-right"></i></button>
</div>
</div>
</div>
