...
$(function(){
	...
	$.each($('input[name="extra[2]"]'),function(k,v){
		if ($(this).prop('checked')){
		i = k;
		}
	});

	$.each($('input[name="extra[3]"]'),function(k,v){
		if ($(this).prop('checked')){
		j = k;
		}
	});
	...
	$('input[name="extra[2]"]').on('click',function(){
	var ii = $('input[name="extra[2]"]').index(this);
	setMethod(ii,2);
	});

	$('input[name="extra[3]"]').on('click',function(){
	var jj = $('input[name="extra[3]"]').index(this);
	setMethod(jj,3);
	});
...
var methods ={
	...
	for (var i in methods[f]){
		for(var j in methods[f][i]){
			$("#form-group_"+methods[f][i][j]).hide().find('input,select,textarea').prop('disabled',true);
		}
	}

	if (e!==undefined){
		for(var j in methods[f][e]){
			$("#form-group_"+methods[f][e][j]).slideDown().find('input,select,textarea').prop('disabled',false);
			$("#form-group_"+methods[f][e][j]).find('label > span.label').addClass('label-danger').text('必須');
		}
	}
}
</script>
