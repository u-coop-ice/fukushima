...
var methods ={
	...
	if (e!==undefined){
		for(var j in methods[e]){
			$("#form-group_"+methods[e][j]).slideDown().find('input,select,textarea').prop('disabled',false);
			$("#form-group_"+methods[e][j]).find('label > span.label').addClass('label-danger').text('必須');
		}
	}
}
</script>
